...
pub(crate) enum CompactorMessage {
    ...
    PollManifest,
    /// Ticker-triggered message for a [`crate::compaction_worker::CompactionWorker`]
    /// to scan `.compactions` for new `Submitted` entries and claim them.
    PollCompactions,
}
...
impl Compactor {
    ...
    pub async fn run(&self) -> Result<(), crate::Error> {
        ...
        if self.options.embedded_worker {
            let worker = crate::compaction_worker::build_embedded_worker(
                self.manifest_store.clone(),
                self.compactions_store.clone(),
...
impl MessageHandler<CompactorMessage> for CompactorEventHandler {
    ...
    async fn handle(&mut self, message: CompactorMessage) -> Result<(), SlateDBError> {
        match message {
            ...
            // Coordinator does not poll `.compactions` for claims; this variant
            // is dispatched only on the [`crate::compaction_worker::CompactionWorker`].
...
mod tests {
    ...
    async fn test_commit_compacted_entries_writes_manifest() {
        ...
        // Use deterministic ULIDs so retain_active_and_last_finished's
        // max-ID rule produces a stable ordering across the L0 → SR0 → SR1
        // chain below.
